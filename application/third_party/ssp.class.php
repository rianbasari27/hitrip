...
class SSP
{
    ...
    static function order($request, $columns, $isJoin = false)
    {
        ...
        if (isset($request['order']) && count($request['order'])) {
            $orderBy = array();
            $dtColumns = SSP::pluckOrder($columns, 'dt');
            ...
            for ($i = 0, $ien = count($request['order']); $i < $ien; $i++) {
                ...
                if ($requestColumn['orderable'] == 'true') {
                    $dir = $request['order'][$i]['dir'] === 'asc' ?
                        'ASC' :
                        'DESC';
                    $orderField = isset($column['as']) ? $column['as'] : $column['db'];
        ...
        if (isset($request['search']) && $request['search']['value'] != '') {
            ...
            for ($i = 0, $ien = count($request['columns']); $i < $ien; $i++) {
                ...
                if ($requestColumn['searchable'] == 'true') {
                    $fieldToFilter = $column['db'];
                    $complexDb = strpos($column['db'], "AS `");
                    if ($complexDb) {
                        $fieldToFilter = substr_replace($fieldToFilter, "", $complexDb);
                    }
                    $binding = SSP::bind($bindings, '%' . $bindStr . '%', PDO::PARAM_STR);
                    $globalSearch[] = ($isJoin) ? $fieldToFilter . " LIKE " . $binding : $fieldToFilter . " LIKE " . $binding;
                }
            }
        }
     ...
     *
     */
    static function simple($request, $sql_details, $table, $primaryKey, $columns, $joinQuery = NULL, $extraWhere = '', $groupBy = '', $having = '', $tableAlias = null)
    {
        $bindings = array();
        ...
        if ($tableAlias) {
            $tableStatement = $table . " AS " . $tableAlias;
        } else {
    ...
    static function pluckOrder($a, $prop, $isJoin = false)
    {
        $out = array();
        for ($i = 0, $len = count($a); $i < $len; $i++) {
            //     $out[] = ($isJoin && isset($a[$i]['as'])) ? $a[$i][$prop] . ' AS ' . $a[$i]['as'] : $a[$i][$prop];
