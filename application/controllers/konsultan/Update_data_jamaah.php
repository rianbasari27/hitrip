...
class Update_data_jamaah extends CI_Controller
{
    ...
    {   
        ...
        if (empty($data_member)) {
            $this->alert->setJamaah('red', 'Error!', 'Data Tidak Ditemukan');
            redirect(base_url() . 'konsultan/home');
            return false;
        }
        ...
        $data_jamaah->provinceList = $province;
        ...
        $this->load->view('konsultan/update_jamaah', $data);
    }
    ...
    public function proses_update_data()
    {
        ...
        $unsetJamaah = ['id_member', 'id_paket' ,'paspor_no', 'paspor_name', 'paspor_issue_date', 'paspor_expiry_date', 'paspor_issuing_city', 'paspor_scan', 'paspor_scan2',
                        'ktp_scan', 'foto_scan', 'visa_scan', 'kk_scan', 'paspor_check', 'buku_kuning_check', 'foto_check', 'id_agen', 'pilihan_kamar', 'verified1', 'pernah_umroh', 'sharing_bed'];
        ...
        $this->load->model('registrasi');
        $dataMember = $this->registrasi->getMember(null, $_POST['id_jamaah']);
        $this->load->model('registrasi');
        if (empty($dataMember)) {
            $registerFrom = null;
        } else {
        ...
        $redir_string = base_url() . 'konsultan/jamaah_info?id='. $dataMember[0]->idSecretMember;
